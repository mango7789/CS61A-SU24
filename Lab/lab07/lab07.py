...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        num_year = 1
        while True:
            total_balance = self.balance * (1 + self.interest) ** num_year
...
class Transaction:
    ...
    def report(self):
        ...
        if self.changed():
            "*** YOUR CODE HERE ***"
            state = "decreased" if self.before > self.after else "increased"
            msg = state + " " + str(self.before) + "->" + str(self.after)
...
class BankAccount:
    ...
    def deposit(self, amount):
        ...
        """
        self.balance = self.balance + amount
        curr_trans = Transaction(len(self.transactions), self.balance - amount, self.balance)
        self.transactions.append(curr_trans)
        return self.balance
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        "*** YOUR CODE HERE ***"
        coins_list = make_change(coin, dict(self.coins))
        if coins_list is None:
            return [coin]
        else:
            for coin_val in coins_list:
                self.coins[coin_val] -= 1
                if self.coins[coin_val] == 0:
                    self.coins.pop(coin_val)
