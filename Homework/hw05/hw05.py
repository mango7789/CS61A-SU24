...
class Player:
    ...
    def check_negative(cls):
        if cls.popularity < 0:
            cls.popularity = 0
    
    def debate(self, other):
        "*** YOUR CODE HERE ***"
        gain_prob = max(0.1, self.popularity / (self.popularity + other.popularity))
        rand_prob = self.random_func()
        if gain_prob > rand_prob:
            self.popularity += 50
        else:
            self.popularity -= 50
            Player.check_negative(self)
...
class Game:
    ...
    def play(self):
        while not self.game_over():
            "*** YOUR CODE HERE ***"
            if self.turn % 2 == 0:
                p1_strategy = self.p1.choose(self.p2)
                p1_strategy(self.p2)
            else:
                p2_strategy = self.p2.choose(self.p1)
                p2_strategy(self.p1)
            self.turn += 1
        return self.winner()
...
def add_d_leaves(t, v):
    ...
    """
    "*** YOUR CODE HERE ***"
    def add_leaves(t, count):
        for branch in t.branches:
            add_leaves(branch, count + 1)
...
def crispr_gene_insertion(lnk_of_genes, insert):
    ...
    def insert_gene(lnk, index):
        ...
        insert_gene(lnk.rest, index + 1)
        
    insert_gene(lnk_of_genes, 0)  
...
def transcribe(dna):
    ...
    return [["".join(dict[dna[gene]]) for gene in range(index, index + 3)] for index in range(0, len(dna), 3)]
