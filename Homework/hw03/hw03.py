...
def balanced(m):
    ...
    m_left = left(m)
    m_right = right(m)
    return total_mass(end(m_left)) * length(m_left) == total_mass(end(m_right)) * length(m_right) \
            and (balanced(end(m_left)) and balanced(end(m_right)))
