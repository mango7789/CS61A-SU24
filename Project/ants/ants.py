...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if hasattr(self, 'damage') and self.doubled == False:
            self.damage *= 2
            self.doubled = True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        # skip the short distance section
        if self.lower_bound != -1:
            while count < self.lower_bound:
                bee_place = bee_place.entrance
                if bee_place.is_hive:
                    return None
                count += 1
        
        while bee_place.bees == []:
            bee_place = bee_place.entrance
            count += 1
            if bee_place.is_hive or count > self.upper_bound:
                return None
            
        return random_bee(bee_place.bees)
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        fire_damage = amount
        # if the ant dies, add extra damage
        if self.health <= amount:
            fire_damage += self.damage
        # cause damage to bees in the same place
        for bee in list(self.place.bees):
            bee.reduce_health(fire_damage)
        # reduce the health of ant
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.turns_to_chew > 0:
            ...
            return
        # Choose a random bee to eat
        else:
            bees = self.place.bees
            if bees == []:
                return
            else:
                chewed_bee = random_bee(bees)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while ant_place is not None:
            curr_ant = ant_place.ant
            if curr_ant is not None:
                curr_ant.double()
                if curr_ant.is_container is True and curr_ant.ant_contained is not None:
